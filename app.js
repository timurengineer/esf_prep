    ...
    request.onload = function() {
        ...
        if (request.status == 200) {
            ...
            if (queryInvoiceResponse.status === "success"){
                errorBox.style.display = 'none';
                //document.getElementById('content').textContent = JSON.stringify(queryInvoiceResponse);
...
function logIn(){
    ...
    request.onload = function() {
        if (request.status == 200) {
            createSessionResponse = JSON.parse(request.responseText);
            if (createSessionResponse.status === "success"){
                errorBox.style.display = 'none';
                document.getElementById('login_page').style.display = 'none';
                document.getElementById('app_page').style.display = 'block';
                document.getElementById('user_company').textContent = "Company: "  + companyInput.options[companyInput.selectedIndex].innerHTML;
